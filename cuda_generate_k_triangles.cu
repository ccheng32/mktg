...
__device__ bool edge_exist(node_t a, node_t b, cuda_edge_t* edges, size_t edge_count) {
...
__global__ void cuda_generate_k_triangles(node_t* nodes, size_t node_count, cuda_edge_t* edges, size_t edge_count, cuda_triangle_t* triangles, size_t* triangle_count) {
  ...
  if (edge_id < edge_count && node_id < node_count) {
    ...
    if (edge_exist(node, edge.a, edges, edge_count) && edge_exist(node, edge.b, edges, edge_count)) {
